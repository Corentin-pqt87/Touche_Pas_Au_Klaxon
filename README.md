...
## Instalatin
Pour installer cette application web, deux moyen sofre a vous:
...
```
##### 3.
Initialiser la base de donnée en vous rendent sur la page phpmyadmin et créer une nouvelle base de donnée nomé `TPAK`.\
Vous trouverez dans [`./Db/dump-TPAK-202607011730.sql`](./Db/dump-TPAK-202607011730.sql) toute la base de donnée initiale.
##### 4.
Ajouter à la racine du projet un fichier nomé `.env` qui aura comme contenue les accé a la base de donnée
*Exemple :*
```
DB_HOST=localhost
DB_PORT=33306
...
### Local
##### 1.
Déplacez vous dans le répèrtoire que vous souéter
##### 2.
Télécharger le projet via GitHub$\to$Code$\to$ [Download Zip](https://github.com/Corentin-pqt87/Touche_Pas_Au_Klaxon/archive/refs/heads/main.zip) ou via la commande suivante
```sh
git clone "https://github.com/Corentin-pqt87/Touche_Pas_Au_Klaxon.git"
...
Crée une nouvelle base de donnée nomé `TPAK`.
...
##### 4.
jouter à la racine du projet un fichier nomé `.env` qui aura comme contenue les accé a la base de donnée
*Exemple :*
```
DB_HOST=localhost
DB_PORT=33306
...
### local
Pour éxécuter le projet, vous trouverez 2 fichiers.
